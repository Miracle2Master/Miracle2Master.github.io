...
| docker inspect [容器名]                  | 查看容器详细信息               | [docker inspect](https://docs.docker.com/engine/reference/commandline/inspect/) |
| docker network connect [网络名] [容器名] | 指定容器加入到网络中           |                                                              |
...
#### docker save 与 docker load — 镜像的导出与导入

在某些场景下，服务器无法直接访问外网（DockerHub），此时就需要在能上网的机器上先把镜像拉下来，导出为压缩包，再拷贝到目标服务器上导入使用。

##### docker save — 将镜像保存为压缩包

```bash
# 语法
docker save -o 导出文件名.tar 镜像名:版本号

# 示例：将 nginx 镜像导出为 nginx.tar
docker save -o nginx.tar nginx:latest

# 也可以同时导出多个镜像
docker save -o multi.tar nginx:latest mysql:8.0 redis:7.0
```

参数说明：
- `-o`：指定导出的文件名（output）
- 也可以使用重定向方式：`docker save nginx:latest > nginx.tar`

##### docker load — 从压缩包加载镜像
...
ls -lh *.tar
# 结果：
# -rw------- 1 root root 192M 7月  21 10:00 nginx.tar
# -rw------- 1 root root 586M 7月  21 10:01 mysql.tar

# 5. 将 tar 文件拷贝到目标服务器（通过 U 盘、scp、内网共享等方式）
scp nginx.tar mysql.tar user@目标服务器IP:/root/
...
# REPOSITORY   TAG       IMAGE ID       CREATED         SIZE
# nginx        latest    605c77e624dd   16 months ago   141MB
# mysql        8.0       3218b38490ce   17 months ago   516MB
...
> **使用场景总结**：
> - **离线环境部署**：内网服务器无法访问外网时，在有网的机器上 docker pull → docker save → 拷贝 → docker load
> - **镜像备份**：将重要镜像导出为文件存档备份
> - **镜像迁移**：将镜像从一台机器迁移到另一台机器
> - **镜像分享**：在没有私有镜像仓库时，通过 tar 包分享镜像

> **注意**：`docker save` 导出的是完整的镜像层数据，文件体积较大。`docker load` 会将压缩包中的所有镜像层完整恢复。
...
![img](img/1778250016236-1.png)

以及对应的初始化SQL脚本和配置文件：

![img](img/1778250016236-2.png)

![img](img/1778250016236-3.png)

其中，hm.cnf主要是配置了MySQL的默认编码，改为utf8mb4；而hmall.sql则是后面我们要用到的黑马商城项目的初始化SQL脚本。

我们直接将整个mysql目录上传至虚拟机的`/root`目录下：

![img](img/1778250016236-4.png)
...
| `-d` | **detach**，后台运行，不阻塞终端 |
| `--name redis` | 给容器取名为 `redis`，方便后续通过 `docker stop redis` 等命令操作 |
| `-p 6379:6379` | **端口映射**，将宿主机的 6379 映射到容器的 6379。6379 是 Redis 默认服务端口，映射后宿主机可通过 `localhost:6379` 连接 Redis |
| `-p 8001:8001` | 第二个端口映射。8001 是 **RedisInsight**（Redis Stack 自带的 Web 可视化管理界面）端口，浏览器访问 `localhost:8001` 即可使用 |
| `-e REDIS_ARGS="--requirepass 123456"` | **环境变量**，传入 Redis 启动参数。`--requirepass 123456` 表示设置访问密码为 `123456`，客户端连接后需执行 `AUTH 123456` 才能操作 |
| `redis/redis-stack:latest` | 使用的镜像。`redis/redis-stack` 是 Redis 官方发布的 **Redis Stack** 镜像（含 RedisJSON、RediSearch、RedisInsight 等扩展模块），`latest` 表示最新版本 |

**Redis Stack 与普通 Redis 的区别：**

| 模块 | 用途 |
|------|------|
| RedisJSON | 原生 JSON 文档存储和查询 |
| RediSearch | 全文检索、二级索引 |
| RedisGraph | 图数据库能力 |
| RedisTimeSeries | 时序数据处理 |
| RedisBloom | 布隆过滤器、布谷鸟过滤器 |
| RedisInsight | Web 可视化客户端（对应 8001 端口） |
