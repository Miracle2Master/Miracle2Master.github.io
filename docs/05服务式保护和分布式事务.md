从 外号外！现在点击****[黑马逆袭之路！！！](https://b11et3un53m.feishu.cn/wiki/MnIqwghIsitrrVkeasjcmWA5nhB)****，添加黑马老师即可领取多套试学课程。还可以预约1V1职业规划咨询帮你找准定位、不再盲目。赶快行动起来吧！**
...
![img](img/1778896569194-2.png)

2）运行

将jar包放在任意非中文、不包含特殊字符的目录下，重命名为`sentinel-dashboard.jar`：

![img](img/1778896569194-3.png)
...
![img](img/1778896569194-4.png)

需要输入账号和密码，默认都是：sentinel

登录后，即可看到控制台，默认会监控sentinel-dashboard服务本身：

![img](img/1778896569194-5.png)
...
![img](img/1778896569194-6.png)

点击簇点链路菜单，会看到下面的页面：

![img](img/1778896569194-7.png)
...
![img](img/1778896569194-9.png)

## 1.3.请求限流

在簇点链路后面点击流控按钮，即可对其做限流配置：

![img](img/1778896569194-10.png)

在弹出的菜单中这样填写：

![img](img/1778896569194-11.png)

这样就把查询购物车列表这个簇点资源的流量限制在了每秒6个，也就是最大QPS为6.

我们利用Jemeter做限流测试，我们每秒发出10个请求：

![img](img/1778896569194-12.png)

最终监控结果如下：

![img](img/1778896569194-13.png)
...
![img](img/1778896569194-14.png)

### 1.4.2.配置线程隔离

接下来，点击查询商品的FeignClient对应的簇点资源后面的流控按钮：

![img](img/1778896569194-15.png)

在弹出的表单中填写下面内容：

![img](img/1778896569194-16.png)

注意，这里勾选的是并发线程数限制，也就是说这个查询功能最多使用5个线程，而不是5QPS。如果查询商品的接口每秒处理2个请求，则5个线程的实际QPS在10左右，而超出的请求自然会被拒绝。

![img](img/1778896569194-17.png)

我们利用Jemeter测试，每秒发送100个请求：

![img](img/1778896569194-18.png)

最终测试结果如下：

![img](img/1778896569194-19.png)

进入查询购物车的请求每秒大概在100，而在查询商品时却只剩下每秒10左右，符合我们的预期。

此时如果我们通过页面访问购物车的其它接口，例如添加购物车、修改购物车商品数量，发现不受影响：

![img](img/1778896569194-20.png)
...
![img](img/1778896569194-22.png)

**步骤三**：在`hm-api`模块中的`ItemClient`接口中使用`ItemClientFallbackFactory`：

![img](img/1778896569194-23.png)

重启后，再次测试，发现被限流的请求不再报错，走了降级逻辑：

![img](img/1778896569194-24.png)

但是未被限流的请求延时依然很高：

![img](img/1778896569194-25.png)
...
对于商品服务这种不太健康的接口，我们应该停止调用，直接走降级逻辑，避免影响到当前服务。也就是将商品查询接口**熔断**。<u>当商品服务接口恢复正常后，再允许调用</u>。这其实就是**断路器**的工作模式了。
...
![img](img/1778896569194-26.png)

在弹出的表格中这样填写：

![img](img/1778896569194-27.png)
...
![img](img/1778896569194-28.png)

在一开始一段时间是允许访问的，后来触发熔断后，查询商品服务的接口通过QPS直接为0，所有请求都被熔断了。而查询购物车的本身并没有受到影响。

此时整个购物车查询服务的平均RT影响不大：

![img](img/1778896569194-29.png)
...
![img](img/1778896569194-30.png)

目前有4个购物车，然结算下单，进入订单结算页面：

![img](img/1778896569194-31.png)

然后将购物车中某个商品的库存修改为`0`：

![img](img/1778896569194-32.png)

然后，提交订单，最终因库存不足导致下单失败：

![img](img/1778896569194-33.png)

然后我们去查看购物车列表，发现购物车数据依然被清空了，并未回滚：

![img](img/1778896569194-34.png)
...
![img](img/1778896569194-36.png)

### 2.2.2.准备配置文件

课前资料准备了一个seata目录，其中包含了seata运行时所需要的配置文件：

![img](img/1778896569194-37.png)

其中包含中文注释，大家可以自行阅读。

我们将整个seata文件夹拷贝到虚拟机的`/root`目录：

![img](img/1778896569194-38.png)
...
```shell
-p 8099:8099 \ # 微服务和Seata连接时的端口
-p 7099:7099 \ # 控制台端口
-v ./seata:/seata-server/resources \ # 挂载文件
...
![img](img/1778896569195-42.png)

结果：

![img](img/1778896569195-43.png)
...
![img](img/1778896569195-45.png)

异常情况：

![img](img/1778896569195-46.png)
