...
![img](img/1784600401333-3.png)

查看docker容器，发现都正常启动了：

![img](img/1784600401333-4.png)

由于采用的是host模式，我们看不到端口映射。不过能直接在宿主机通过ps命令查看到Redis进程：

![img](img/1784600401333-5.png)
...
> | 部分 | 说明 |
> |------|------|
> | `docker exec` | 在**运行中的容器**内执行命令 |
> | `-it` | 两个参数组合：`-i`（interactive，保持标准输入打开）+ `-t`（tty，分配一个伪终端），合在一起让你能**交互式地**操作容器内命令 |
> | `r2` | 目标容器的名称，在这里是一个 Redis 容器 |
> | `redis-cli` | 容器内执行的命令，即 Redis 自带的命令行客户端工具 |
> | `-p 7002` | Redis CLI 的参数，指定要连接的 Redis 服务端口号为 **7002**（Redis 默认端口是 6379） |
...
![img](img/1784600401333-8.png)

再看下`r2`节点执行`replicaof`命令时的日志：

![img](img/1784600401333-9.png)
...
![img](img/1784600401333-11.png)

slave与master的offset之间的差异，就是salve需要增量拷贝的数据了。

随着不断有数据写入，master的offset逐渐变大，slave也不断的拷贝，追赶master的offset：

![img](img/1784600401333-12.png)

直到数组被填满：

![img](img/1784600401333-13.png)

此时，如果有新的数据写入，就会覆盖数组中的旧数据。不过，旧的数据只要是绿色的，说明是已经被同步到slave的数据，即便被覆盖了也没什么影响。因为未同步的仅仅是红色部分：

![img](img/1784600401333-14.png)

但是，如果slave出现网络阻塞，导致master的`offset`远远超过了slave的`offset`： 

![img](img/1784600401333-15.png)

如果master继续写入新数据，master的`offset`就会覆盖`repl_baklog`中旧的数据，直到将slave现在的`offset`也覆盖：

![img](img/1784600401333-16.png)
...
![img](img/1784600401334-37.png)

过了一段时间后，7002原本的小弟7006变成了`master`：

![img](img/1784600401334-38.png)
...
![](img/image-20260722160400389.png)
...
![image-20260722161203303](img/image-20260722161203303.png)

我们先分析策略1，先更新数据库再删除缓存：

**正常情况**

![img](img/1784600401334-46.png)

**异常情况**

![img](img/1784600401334-47.png)
...
![img](img/1784600401334-48.png)

**异常情况**

![img](img/1784600401334-49.png)
...
![img](img/1784600401334-53.png)

## 6.3.缓存雪崩

缓存雪崩是指在同一时段大量的缓存key同时失效或者Redis服务宕机，导致大量请求到达数据库，带来巨大压力。

![img](img/1784600401334-54.png)
...
![img](img/1784600401334-56.png)

逻辑过期的思路如图：

![img](img/1784600401334-57.png)
